...
namespace alg
{
    ...
    namespace detail
    {
        ...
        private:
            ...
            static constexpr auto limit = tmp::math::power<2,tmp::number_of_bits_of_fundamental<std::size_t> / 2> - 1;
        ...
        private:
            ...
            void init_is_primes()
            {
                for(unsigned int i = 2; i <= sqrt_N; ++i)
                {
                    if(!isnt_prime[i])
                    {
                        for(unsigned int j = i*i; j <= sqrt_N; j+=i*i)
                        {
                            isnt_prime[j] = true;
                        }
                    }
    ...
    namespace detail
    {
        ...
        private:
            ...
            enum { limit = tmp::math::power<2, tmp::number_of_bits_of_fundamental<std::size_t> / 2> - 1 };
            enum { sqrt_N = find_sqrt_N(0,limit) };
            bool isnt_prime[sqrt_N+1] = {0};
        ...
        public:
            bool prime() const { return prime_; }
            eratosthenes_sieve_static_primes()
            {
                init_is_primes();
                prime_ = isprime_();
            }
